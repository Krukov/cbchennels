...
class SimpleSerializer(object):
    ...
    def data(self):
        if not self._data:
            if self.many:
                data = [model_to_dict(instance, **self.kwargs) for instance in self.instance]
            else:
...
class DjangoSerializer(SimpleSerializer):
    ...
    def data(self):
        if not self._data:
            self._data = serializers.serialize(self.kwargs.get('format', 'json'),
                                               self.instance if self.many else [self.instance],
                                               **self.kwargs)
        return self._data
